...
export const { GET, POST } = createChatHandler({
  ...
  onLead: async (lead) => {
    console.log('[onLead] classification:', lead.classification, '| confidence:', lead.confidence);
    console.log('[onLead] visitor:', lead.visitor);
    console.log('[onLead] brief:', lead.brief);
  },

  onChatEnd: async ({ classification, confidence, transcript }) => {
    console.log(
      `[onChatEnd] classification=${classification} confidence=${confidence} turns=${transcript.length}`
    );
  },
});
